 ...
 * Add `symfony-profiler-assert` tool that checks a profile against expectations (`maxQueries`, `maxDurationMs`, `maxDuplicates`, `expectNoException`) and returns `passed` with every expectation as actual against limit, plus the most repeated statement groups when a query budget is missed
