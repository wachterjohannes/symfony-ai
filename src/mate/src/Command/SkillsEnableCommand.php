...
HELP
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if ([] !== $statuses && !$statuses[0]->enabled) {
            $io->warning(\sprintf('Skill "%s" stays hidden because the extension "%s" is disabled in mate/extensions.php.', $statuses[0]->installedName, $resolved['package']));
            $this->renderSkillRow($io, $this->manager, $resolved['name']);

            return Command::SUCCESS;
        }

        $io->success(\sprintf('Skill "mate-%s" is enabled and was installed.', $resolved['name']));
        $this->renderSkillRow($io, $this->manager, $resolved['name']);
