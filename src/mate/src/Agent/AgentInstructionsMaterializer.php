...
/**
 * Writes the managed Mate block into `AGENTS.md` and the `CLAUDE.md` import that makes Claude
 * Code read it.
 *
 * The detailed tool guidance lives in the skills each extension ships, not here.
 *
...
final class AgentInstructionsMaterializer
{
    ...
    /**
     * The file older versions generated. It is no longer written or read; it is only looked up
     * so `discover` can point out a leftover copy.
     */
    public const LEGACY_INSTRUCTIONS_FILE = 'mate/AGENT_INSTRUCTIONS.md';
    ...
    /**
     * @param array<string, ExtensionData>|null $extensions null when the installed extensions are
     *                                                      not known yet, as in `mate init`
     *
     * @return MaterializationResult
     */
    public function materializeForExtensions(?array $extensions = null): array
    {
        return [
            'agents_file_updated' => $this->writeAgentsFile($extensions),
            'claude_file_updated' => $this->writeClaudeFile(),
        ];
    }

    public function hasLegacyInstructionsFile(): bool
    {
        return file_exists($this->rootDir.'/'.self::LEGACY_INSTRUCTIONS_FILE);
    }
