 ...
 * Add `Capability::CLASSIFICATION`, the `Classification` namespace (`BooleanQuestion`, `ChoiceQuestion` and `ScoreQuestion` with their typed `BooleanAnswer`, `ChoiceAnswer` and `ScoreAnswer`) and `Result\ClassificationResult` for calibrated classification of a state against a set of typed questions
