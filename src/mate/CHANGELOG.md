 ...
 * Fix `mate init` pinning `mate.php_version` from the PHP version of the process running `init` itself, even when the answered `mate.invocation` wraps the binary in a container/multi-PHP prefix (`ddev exec`, `symfony php`, ...) that runs under a different interpreter; it now runs `php` through that wrapper to detect the version it actually executes with, falling back to the current process's version and warning when the wrapper cannot be probed
