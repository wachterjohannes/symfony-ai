 ...
 */
final class InvestigatePerformanceRecipeTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->tempCacheDir = sys_get_temp_dir().'/ai_mate_recipe_perf_'.uniqid();
    ...
    {
        $this->assertSame('investigate_performance', (new InvestigatePerformanceRecipe())->name());
    }

    public function testPropagatesProfilerUnavailableWarning()
    {
        // Build a toolbox without RequestGraphFactory/ProfilerDataProvider — recipe must
        // surface the actionable warning instead of pretending to compose further steps.
    ...
    public function testProducesEnvelopeForKnownTokenViaRequestGraph()
    {
        ...
        $result = (new InvestigatePerformanceRecipe())->run(['target' => 'request:abc123'], $toolBox);

        $this->assertSame('request:abc123', $result->primaryNodes[0]['id'] ?? null);
        $this->assertStringContainsString('investigate_performance', $result->summary);
    }

    private function buildProfilerData(): ProfilerDataProvider
    {
        ...
        return new ProfilerDataProvider($this->fixturesDir.'/profiler', $registry);
    }
