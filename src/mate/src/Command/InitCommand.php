...
class InitCommand extends Command
{
    ...
    public function __construct(
        ...
        private InvocationPhpVersionProbe $phpVersionProbe = new InvocationPhpVersionProbe(),
    ) {
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if ($this->phpVersionProbe->isWrapped($this->invocation)) {
            ...
            if (null !== $detectedVersion) {
                ...
                $io->warning(\sprintf(
                    'Could not determine which PHP version "%s" actually runs. Pinned "mate.php_version" to the current process\'s PHP "%s" instead; if that is not what "%s" runs, edit "mate.php_version" in mate/config.php by hand.',
                    $this->invocation,
                    $this->phpVersion,
                    $this->invocation,
                ));
